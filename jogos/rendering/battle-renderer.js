...
const assets = {
    cenario:       null,
    cenario1:      null,
    cenario_glacial: null,
    cenario_inferno: null,
    chao:          null,
...
function carregarAssets() {
    ...
    _img('piso-de-batalha/cenario1.png', i => { assets.cenario1 = i; assets.cenario = i; }, 'Cenário 1 não carregado');
    _img('piso-de-batalha/cenario_glacial.png', i => assets.cenario_glacial = i, 'Cenário Glacial não carregado');
    _img('piso-de-batalha/cenario_inferno.png', i => assets.cenario_inferno = i, 'Cenário Inferno não carregado');
    ...
    _img('monstros/pecado_orgulho.png', i => assets.orgulho = i, 'Monstro Orgulho não carregado');
    _img('monstros/pecado_inveja.png', i => assets.inveja = i, 'Monstro Inveja não carregado');
    _img('monstros/pecado_avareza.png', i => assets.avareza = i, 'Monstro Avareza não carregado');
    _img('monstros/pecado_ira.png', i => assets.ira = i, 'Monstro Ira não carregado');
}

// ════════════════════════════════════════
//  ALTERNÂNCIA DE MONSTROS
// ════════════════════════════════════════
// 'slime', 'saga', 'orgulho' ou 'inveja'
let monstroAtivo = 'slime';

EventBus.on('inimigo:configurado', (dados) => {
    const tipo = tiposMonstros[dados.tipo] || tiposMonstros[0];
    monstroAtivo = tipo.asset || 'slime';
    
    // Atualiza cenário baseado no estágio
    const cenarioKey = getCenarioEstagio(dados.nivel);
    if (assets[cenarioKey]) {
        assets.cenario = assets[cenarioKey];
    }
});
...
EventBus.on('inimigo:dano', () => {
    // Feedback universal de trepidação e flash
    hit.tremendo = 18;
    hit.flash    = 10;
    ...
    // Efeito de partículas de impacto no centro do monstro
    _criarImpacto(canvas);
});

function _criarImpacto(canvas) {
    const mx = monstroX(canvas);
    const my = chaoY(canvas) - canvas.height * 0.2;
    for (let i = 0; i < 8; i++) {
        particulas.push({
            x: mx,
            y: my,
...
function _desenharMonstro(ctx, canvas) {
    ...
    } else if (monstroAtivo === 'slime') {
        _desenharSlime(ctx, canvas);
    } else {
        // Novos monstros (orgulho, inveja, avareza, ira)
        _desenharNovoMonstro(ctx, canvas);
    }
}

function _desenharNovoMonstro(ctx, canvas) {
    if (hit.flash    > 0) hit.flash--;
    if (hit.tremendo > 0) hit.tremendo--;

    const ini = BattleState.inimigo;
    if (!ini || ini.maxHp <= 0) return;

    const pct = Math.max(0.05, ini.hp / ini.maxHp);
    // Monstros novos são maiores e mais imponentes
    const tam = canvas.height * (0.75 + pct * 0.1);

    // Trepidação mais agressiva e randômica
    const ox = hit.tremendo > 0 ? (Math.random() - 0.5) * 30 : 0;
    const oy = hit.tremendo > 0 ? (Math.random() - 0.5) * 20 : 0;

    const mx = monstroX(canvas) + ox;
    const my = chaoY(canvas) - tam * 0.45 + oy;

    const img = assets[monstroAtivo];

    ctx.save();
    
    // Efeito de brilho externo (aura)
    ctx.shadowBlur  = 60 + Math.sin(Date.now() * 0.006) * 25;
    ctx.shadowColor = pct < 0.25 ? 'rgba(255, 50, 50, 1)' : 'rgba(150, 50, 255, 0.9)';

    if (imgOk(img)) {
        const lar = tam * (img.naturalWidth / img.naturalHeight);
        
        // REMOVER FUNDO PRETO (Chroma Key manual no canvas)
        // Como as imagens geradas têm fundo preto puro, vamos usar blend mode
        // 'screen' remove o preto e mantém as cores claras.
        ctx.globalCompositeOperation = 'screen';

        // Se estiver levando dano, aplica um flash branco épico
        if (hit.flash > 0) {
            ctx.save();
            ctx.filter = 'brightness(5) saturate(0)'; // Flash branco puro e intenso
            ctx.globalAlpha = hit.flash / 10;
            ctx.drawImage(img, mx - lar / 2, my - tam / 2, lar, tam);
            ctx.restore();
            
            // Desenha a imagem original com um tom avermelhado por baixo do flash
...
function _criarParticula(x, y) {
    ...
    particulas.push({
        ...
        size: useEmoji ? 16 + Math.random() * 8 : 6 + Math.random() * 10,
        cor:  `hsl(${320 + Math.random() * 80},100%,70%)`,
...
function render(ctx, canvas) {
    ...
    particulas.forEach(p => {
        p.x += p.vx;
        p.y += p.vy;
        p.vy += p.magica ? 0.03 : (p.flash ? 0.35 : 0.14);
        p.vida--;
    });
    ...
    textos.forEach(t => {
        ...
        const isCrit = t.tipo === 'critico';
        const [cor, tam] = t.tipo === 'levelup' ? ['#ffd700', 32]
                         : t.tipo === 'dps'     ? ['#aaffaa', 16]
                         : isCrit               ? ['#ff0000', 36]
                         : ['#ff9de2', 24];
        ...
        ctx.font = `bold ${tam}px 'Cinzel', serif`;
        ctx.strokeStyle = 'rgba(0,0,0,0.95)'; ctx.lineWidth = isCrit ? 6 : 4;
