...
function _desenharNovoMonstro(ctx, canvas) {
    ...
    const pct = Math.max(0.05, ini.hp / ini.maxHp);
    const tam = canvas.height * (0.60 + pct * 0.05);

    const ox = hit.tremendo > 0 ? (Math.random() - 0.5) * 18 : 0;
    const oy = hit.tremendo > 0 ? (Math.random() - 0.5) * 10 : 0;

    const mx = monstroX(canvas) + ox;
    const my = chaoY(canvas) - tam * 0.40 + oy;

    const img = assets[monstroAtivo];

    ctx.save();
    ctx.shadowBlur  = 42 + Math.sin(Date.now() * 0.004) * 14;
    ctx.shadowColor = pct < 0.25 ? 'rgba(255,220,0,1)' : 'rgba(200,100,255,0.8)';
    ...
    if (imgOk(img)) {
        ...
        // Fallback genérico se a imagem falhar
        ctx.fillStyle = '#ff00ff';
        ctx.beginPath();
        ctx.arc(mx, my, tam * 0.2, 0, Math.PI * 2);
        ctx.fill();
    }
